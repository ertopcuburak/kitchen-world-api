...
public interface RecipeMaterialRepository extends JpaRepository<RecipeMaterial, Integer> {
    @Query(value = "SELECT rm.id, rm.recipe_id, rm.material_id, rm.quantity, ma.name as material_name, ma.is_banned as is_banned FROM recipe_material as rm INNER JOIN material as ma ON rm.material_id = ma.id WHERE rm.recipe_id = ?1", nativeQuery = true)
