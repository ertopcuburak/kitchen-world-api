...
public interface RecipeRepository extends JpaRepository<Recipe, Integer> {
    ...
    @Query(value = "SELECT DISTINCT recipe.* FROM recipe INNER JOIN recipe_material ON recipe_material.recipe_id = recipe.id LEFT JOIN material ON recipe_material.material_id = material.id WHERE material.id IN (?1) GROUP BY recipe.id ORDER BY (COUNT(*) / (SELECT COUNT(*) FROM recipe_material WHERE recipe_id = recipe.id)) DESC LIMIT 7", nativeQuery = true)
    ...
    @Query(value = "SELECT name FROM category WHERE id = ?1 ORDER BY id DESC", nativeQuery = true)
    String getRecipeCategoryName(Integer categoryId);
